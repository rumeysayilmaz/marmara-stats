#!/home/administrator/myprojects/rumeysa_yilmaz/database/venv
...
while True:
    try:
        ...
        print("Current block height from rpc call: " + str(block))
        ...
        if not last_record:  # if no record
            ...
            print("Next Height to be inserted to db: " + str(Height))
        if Height <= (block - 5):
            ...
            if Height > 1:
                CalculatedTotalNormals = TotalNormals - SpentNormals + float(
                    last_record[0][8])  # last_record[0][8] CalculatedTotalNormals from last record
                CalculatedTotalActivated = TotalActivated - SpentActivated + float(
                    last_record[0][9])  # last_record[0][9] CalculatedTotalActivated from last record
                CalculatedTotalLockedInLoops = TotalLockedInLoops - SpentLockedInLoops + float(
                    last_record[0][10])  # last_record[0][10] CalculatedTotalLockedInLoops from last record
                insert_marmara_stat_query = "INSERT INTO " \
                                            "marmarastat(Height, TotalNormals, TotalActivated, TotalLockedInLoops, " \
                                            "SpentNormals, SpentActivated, SpentLockedInLoops, BlockTime, " \
                                            "CalculatedTotalNormals, CalculatedTotalActivated, CalculatedTotalLockedInLoops) " \
                ...
                marmaradb.close_connection(stat_last_record_connection, stat_last_record_cursor)
        if Height > (block - 5):
            print("Entered sleep mode for 30 seconds until the height is 5 blocks away from block height")
            time.sleep(30)
